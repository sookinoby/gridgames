...
angular.module('gridGameThreeGrid', ['gridGameThreeGameData'])
.factory('GenerateUniqueId', function(gameDataService) {
...
//console.log(this);
  this.setStartingTiles = function(num) {
    ...
    if(typeof Array.prototype.indexOf === 'function') {
      ...
      return cell.x >= 0 && cell.x < this.column &&
              cell.y >= 0 && cell.y < this.row;
    };
   ...
   // console.log(neighbhourCellsAvailable);
    ...
    this.insertTileInOrder = function(optionsArrayList,a) {
        ...
        for(var i=0; i < this.row ; i++)
        {
            for(var j=0; j < this.column ; j++)
            {
             avaiableNeighbhourCells.push({x:j,y:i});   
            }
        }    
    ...
    this.storeAnswer = function (tileDetail)
    {
      ...
      var location = service._coordinatesToPosition({x:c_x,y:c_y});
      var index = service.storeSelectedPositions.indexOf(location);
     if( index == -1)
      {  
        if(service.linenumber == 3)
            return;
        tileDetail.flip();
        service.storeSelectedPositions.push(service._coordinatesToPosition({x:c_x,y:c_y}));
        var tile = service.getCellAt({x:c_x,y:c_y})
    ...
    //      console.log(service.factContent);
      ...
      //    console.log(temp_factContent);
        ...
        if(service.storeSelectedPositions.length !==0)
    ...
    this.evaluateAnswer = function() {
      ...
      var guessed_answer = this.getCellAt({x:vector.x,y:vector.y}); 
